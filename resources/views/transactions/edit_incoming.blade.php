                                ...
                                <table class="min-w-full divide-y divide-slate-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-slate-200">
                                        <template x-for="(item, index) in items" :key="index">
                                            <tr>
                                                <td class="px-4 py-3">
                                                    <select :name="'products['+index+'][product_id]'" x-model="item.product_id" @change="updateUnit(index)" required class="block w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                                        <option value="">-- Pilih Produk --</option>
                                                        <template x-for="prod in products" :key="prod.id">
                                                            <option :value="prod.id" x-text="prod.name + ' (Stok: ' + prod.current_stock + ')'"></option>
                                                        </template>
                                                    </select>
                                                </td>
                                                <td class="px-4 py-3">
                                                ...
                                                <td class="px-4 py-3 text-center text-sm">
                                                    <span x-text="item.unit" class="text-gray-500"></span>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('transactionForm', () => ({
                ...
                // Gunakan variabel PHP $detailItems yang sudah disiapkan
                ...
                addRow() { this.items.push({ product_id: '', quantity: '', unit: '' }); },
